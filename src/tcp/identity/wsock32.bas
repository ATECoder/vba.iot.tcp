...
' A data structure that is to receive details of the Windows Sockets implementation.
Public Type WSADATA
    wVersion As Integer
...
' terminates use of the Winsock dll. The return value is zero if the operation was successful.
' Otherwise, the value SOCKET_ERROR is returned, and a specific error number can be retrieved
...
Public Declare PtrSafe Function Socket Lib "wsock32.dll" Alias "socket" (ByVal addressFamily As Long, ByVal socketType As Long, ByVal protocol As Long) As Long
